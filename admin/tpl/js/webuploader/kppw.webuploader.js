...
* @Copyright (c) 2007-2014 http://www.iyun720.com All rights reserved.
*/
			...
			var self = this;
				...
				if(!self.options.uploadEvents.fileQueued){
					self.BDUploader.on('fileQueued',function(file ){
						...
						if(self.allowNum == 0 || self.isAllowUpload() == 0 ){
							...
							art.dialog.alert('上传文件数量超出限制:最多上传'+self.options.fileNumLimit+'个文件');
							return false;
						}
				...
				if(!self.options.uploadEvents.error){
					self.BDUploader.on('error',function(type){
						...
						art.dialog.alert(title,msg,errtype);
					});
				}
				/**
				 * 默认文件上传出错时处理
