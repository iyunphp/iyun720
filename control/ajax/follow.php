...
if($op=='add'){
	...
	if($countFollow){
		...
		db_factory::query (sprintf('update %s set focus_num=focus_num+1 where uid = %d',TABLEPRE.'witkey_shop',intval($id)));
		$arrFd['uid'] = $gUid;
	...
	$followInfo = db_factory::get_one("select * from ".TABLEPRE."witkey_free_follow where ".'uid = '.intval($gUid).' and fuid = '.intval($id));
	if(!$followInfo){
		exit('无权操作');
	}
	db_factory::query (sprintf('update %s set focus_num=focus_num-1 where uid = %d',TABLEPRE.'witkey_shop',intval($id)));
