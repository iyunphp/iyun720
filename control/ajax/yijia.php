...
$wid = intval($wid);
$workInfo = db_factory::get_one('select * from '.TABLEPRE.'witkey_task_work where work_id='.$wid);
$to_username = $workInfo['username'];
$taskInfo =  db_factory::get_one('select * from '.TABLEPRE.'witkey_task where task_id='.intval($workInfo['task_id']));
...
switch($op){
	case 'agree':
		if($gUid == $workInfo['uid']){
			...
			$res = db_factory::updatetable(TABLEPRE.'witkey_task_work',$setArr,array('work_id'=>$wid));
		}else{
			$msg = '非法操作';
		}
		$arr = array('msg'=>$msg);
		...
		$title = '稿件议价';
		$content= '尊敬的'.$to_username.':'.$from_username.$msg.'您的议价。订单详情：<a href="'.$url.'">'.$taskInfo['task_title'].'</a><p>留言：'.$txt_content.'</p>感谢对我们的信任。如有特殊情况，请致电客服 ';
		if (strtoupper ( CHARSET ) == 'GBK') {
...
if (isset ( $formhash ) && kekezu::submitcheck ( $formhash )) {
	$res = db_factory::updatetable ( TABLEPRE . "witkey_task_work", array (
							"hasyj" => 2,
