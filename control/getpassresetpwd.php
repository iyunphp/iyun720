...
if($json == 'json'){
	if(strlen($newpwd) < 6){
		$tips ['newpwd'] = '密码必须大于等于6位数';
	}
	if($newpwd != $newpwd2){
		$tips ['newpwd2'] = '您输入的密码与确认密码不一致';
	}
	if(!empty($tips))
	{
		$json = array(
	...
	else
	{
		...
		$sql1 = sprintf ( "update %switkey_member set password = '%s'  where uid=%d", TABLEPRE, $user_code, $update_uid  );
		db_factory::execute ( $sql1 );
		$sql2 = sprintf ( "update %switkey_space set password = '%s' where uid=%d", TABLEPRE, $user_code,  $update_uid  );
...
if (kekezu::submitcheck ( $formhash )) {
	...
	$user_code = md5 ( $newpwd );
	$sql1 = sprintf ( "update %switkey_member set password = '%s'  where uid=%d", TABLEPRE, $user_code, $validInfo['userinfo'] ['uid'] );
	db_factory::execute ( $sql1 );
	$sql2 = sprintf ( "update %switkey_space set  password = '%s' where uid=%d", TABLEPRE, $user_code,  $validInfo['userinfo'] ['uid'] );
	db_factory::execute ( $sql2 );
	keke_user_class::user_edit ( $validInfo['userinfo'] ['username'], '', $newpwd, '', 1 );
	keke_user_class::updateGetPwdStatusByGetUid($validInfo['userinfo'] ['uid'], 1);
	unset($_SESSION['retrieve']);
