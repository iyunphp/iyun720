...
if($json == 'json'){
	...
	$slide = array();
	$type = 'success';
	$sql = "select service_id as mid, title, uid, username, file_path, on_time, views as popular, content as summary from iyun_witkey_service where service_status=2 and (file_path like '%.mp4%' or file_path like '%.mov%') " . $where . " order by mid desc limit " . $start . "," . $limit;
	$keywordDatas = db_factory::query($sql);
	if(empty($keywordDatas))
	{
		$type = 'error';
		$keywordDatas = db_factory::query("select service_id as mid, title, uid, username, file_path, on_time, views as popular, content as summary from iyun_witkey_service where service_status=2 and (file_path like '%.mp4%' or file_path like '%.mov%') order by mid desc limit 5, 10");
	}
...
$strWhere .= " and (file_path like '%.mp4' or file_path like '%.mov')";
if (intval ( $i )) {
