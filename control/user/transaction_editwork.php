...
if($objId&&!$intTaskId){
	$strServiceSql = 'select * from '.TABLEPRE.'witkey_service where service_id = '.$objId;
	...
	$strCustomSql = 'select a.id,a.extdata,b.f_name,b.f_tips,b.f_fixed_len,b.f_min_len,b.f_max_len,b.f_required,b.f_code,b.listorder,b.f_warning from '.TABLEPRE.'witkey_custom_fields_ext a LEFT JOIN '.TABLEPRE.'witkey_custom_fields b ON a.c_id = b.id where a.objid = '.$objId;
	...
	foreach($arrCustom as $k=>$v){
		...
		//改造，$strCustomSql中添加,b.listorder,b.f_warning两个字段
		$arrCustomInfo[$k]['listorder'] = $v['listorder'];
		if(count(explode('###',$v['f_warning']))>1){			
			$arrCustomInfo[$k]['f_warning_select'] = 1;
			$arrCustomInfo[$k]['f_warning'] = explode('###',$v['f_warning']);
		}
		//改造完毕
		
		//$arrCustomInfo[$k]['music_path'] = CommonClass::getFileArrayByPath(',', $arrServiceInfo['file_path']);
	}
	//print_r($arrCustomInfo);
...
if (isset($formhash)&&kekezu::submitcheck($formhash)) {
   ...
   foreach($arrCustomInfo as $k=>$v){
	    foreach($id as $i=>$j){
			if($v['id'] == $j){
				if($v['extdata'] != $_POST['extdata'][$i]){
					$arrUpdateParts['custom'][$j] = $extdata[$i];
					$arrUpdateParts['old_custom'][$j] = $v['extdata'];
				}
			}
	    }  
	}
	...
	if(!intval($pk['service_id'])){
		...
		if($arrServiceInfo['service_status'] =='1'){
			...
			if(!empty($arrUpdateParts)){				
				$logId = CommonClass::createEditLog($pk['service_id'], $arrServiceInfo['model_id'], serialize($arrUpdateParts));
				/*print_r($log_data);
				die();*/
				...
				createXML($service_id,$arrLogDatas['file_path']);//重新生成xml和全景图片
				//keke_msg_class::send_private_message ( '服务审核通知', '您编辑的作品信息<a href="index.php?do=goods&id=' . $service_id . '">' . $arrServiceInfo ['title'] . '</a>已通过管理员审核，目前已生效！', $arrServiceInfo ['uid'], $arrServiceInfo ['username'] );
				
				CommonClass::cancleEdit ( $service_id, 6 );
				keke_shop_release_class::updateEditStatusBySid($service_id, 4);
				//keke_msg_class::send_private_message ( '服务审核通知', '您编辑的作品信息<a href="index.php?do=goods&id=' . $service_id . '">' . $arrServiceInfo ['title'] . '</a>未通过管理员的审核，如有疑问请联系网站管理员！', $arrServiceInfo ['uid'], $arrServiceInfo ['username'] );
				
				//改造完毕
				
				
			}else{									
				createXML($pk['service_id']);
