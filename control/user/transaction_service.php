...
$serInfo = db_factory::get_one("select * from ".TABLEPRE."witkey_service where service_id =".intval($objId));
if (isset ( $action )) {
	switch ($action) {
		case 'grounding':
			db_factory::execute("update ".TABLEPRE."witkey_service set service_status=1 where service_id=".intval($objId));
			kekezu::show_msg ( '请等待管理员审核，该作品重新变成待审核状态，等待管理员审核', $strUrl, NULL, NULL, 'ok' );
			break;
		case 'undercarriage':
			db_factory::execute("update ".TABLEPRE."witkey_service set service_status=3 where service_id=".intval($objId));
			kekezu::show_msg ( '操作成功', $strUrl, NULL, NULL, 'ok' );
			break;
		case 'mulitDel' :
			if ($ckb) {
				foreach ($ckb as $k =>$v){
					$serInfo = db_factory::get_one("select * from ".TABLEPRE."witkey_service where service_id =".intval($v));
					if($serInfo){
