<?php keke_tpl_class::checkrefresh('admin/tpl/admin_nav_search', '1452226094' );?><!DOCTYPE HTML>
...
<div class="box quick_link">
        ...
        <div class="box link">
        	...
        	<?php if($nav_search) { ?>
<ul>
        		<?php if(is_array($nav_search)) { foreach($nav_search as $k => $v) { ?>
            	<li><a href="javascript:s('<?php echo $v['resource_url'];?>')"><span class="font_st">·</span><?php echo $v['resource_name'];?></a></li>
            	<?php } } ?>
</ul>
<?php } else { ?>
<?php echo $_lang['now_no_result'];?>
<?php } ?>
            </div>
        </div>
      </div>  
 ...
 win.display_frame.location = url; 
