<?php keke_tpl_class::checkrefresh('tpl/default/user/transaction_service', '1452577134' );?><!DOCTYPE HTML>
...
<dl class="nav-list">
  ...
  <div class="content-panel">
    <div class="tab">
    	<a href="#" <?php if($op == 'service') { ?> class="selected" <?php } ?>>我的作品</a>
    </div>
    <div class="tab_detail">
     	<!--<ul class="nav nav-pills second-nav">
          <li <?php if(!$intModelId ) { ?> class="active" <?php } ?>><a href="index.php?do=user&view=transaction&op=service" >全部</a></li>
          <li <?php if($intModelId == '6') { ?> class="active" <?php } ?>><a href="index.php?do=user&view=transaction&op=service&intModelId=6" >文件</a></li>
          <li <?php if($intModelId == '7') { ?> class="active" <?php } ?>><a href="index.php?do=user&view=transaction&op=service&intModelId=7" >服务</a></li>
        </ul>-->
    <div class="action-bar">
    <form action="<?php echo $strUrl;?>" method="post" name="searchServiceForm" id="searchServiceForm">
        <div class="action-item">
        <input type="text" placeholder="请输入编号" class="form-control" name="intId" id="intId" value="<?php echo $intId;?>">
        </div>
        <div class="action-item">
        <input type="text" placeholder="请输入名称" class="form-control" name="strTitle" id="strTitle" value="<?php echo $strTitle;?>">
        </div>
        <div class="action-item">
      ...
      <div class="table-responsive">
        ...
        <?php if(is_array($arrLists)) { foreach($arrLists as $k => $v) { ?>
          <tr>
            ...
            <td title="<?php echo $v['title'];?>"><a href="index.php?do=goods&id=<?php echo $v['service_id'];?>"><?php echo kekezu::cutstr($v['title'],16) ?></a></td>
            <td>
              <span class="money">
              	<sub>￥</sub><?php echo $v['price'];?>
              </span>
            </td>
            <td>
            	<?php echo $v['sale_num'];?>
            ...
            <td> <?php if($v['edit_info']['status'] =='1') { ?>更新待审 <?php } else { ?><?php echo $arrStatus[$v['service_status']];?><?php } ?></td>
            <td>
            <?php if($v['service_status']==3) { ?><a href="javascript:grounding('<?php echo $strUrl;?>&objId=<?php echo $v['service_id'];?>&action=grounding&intPage=<?php echo $intPage;?>');void(0);">上架</a><?php } ?>
            <?php if($v['service_status']==2) { ?><a href="javascript:undercarriage('<?php echo $strUrl;?>&objId=<?php echo $v['service_id'];?>&action=undercarriage&intPage=<?php echo $intPage;?>');void(0);">下架</a><?php } ?>
            
            <?php if($v['edit_status'] =='1') { ?>
            	<a href="javascript:opConfirm('<?php echo $strUrl;?>&objId=<?php echo $v['service_id'];?>&action=cancel_editSingle&intPage=<?php echo $intPage;?>','撤销编辑');void(0);">撤销编辑</a>
            <?php } else { ?>
           	 	<?php if($v['service_status']!=3) { ?><a  href="javascript:editWork('<?php echo $v['service_id'];?>',0);void(0);" >编辑</a><?php } ?>
            <?php } ?>
            
            
<?php if($v['service_status']==2&&TOOL=='TRUE') { ?>
    <a  href="javascript:payitem('goods','<?php echo $v['service_id'];?>','<?php echo $uid;?>');void(0);">增值工具</a>
<?php } ?>
            <a href="javascript:opSingle('<?php echo $strUrl;?>&objId=<?php echo $v['service_id'];?>&action=delSingle&intPage=<?php echo $intPage;?>');void(0);">删除</a>
            </td>
          </tr>
         <?php } } ?>
         <?php } else { ?>
         	<tr>
         		<td colspan="7" class="text-center">暂无相关作品</td>
         	</tr>
         <?php } ?>
        </tbody>
      </table>
