<?php keke_tpl_class::checkrefresh('tpl/default/user/wk_gy', '1452161275' );?><!DOCTYPE HTML>
...
<form action="<?php echo $strUrl;?>" method="post" name="SearchForm" id="SearchForm">
<div class="action-item">
<input type="text" placeholder="请输入编号" class="form-control" name="id" id="id" value="<?php echo $id;?>" maxlength="10">
</div>
<div class="action-item">
<input type="text" placeholder="请输入雇主名称" class="form-control" name="gzname" id="gzname" value="<?php echo $gzname;?>" maxlength="30">
</div>
<div class="action-item">
<select class="form-control" name="s" id="s">
<option value="-1">全部</option>
<?php if(is_array($arrStatus)) { foreach($arrStatus as $k => $v) { ?>
<option value="<?php echo $k;?>" <?php if($s==$k) { ?> selected="selected"<?php } ?>><?php echo $v;?></option>
<?php } } ?>
</select>
</div>
<div class="action-item">
...
<option value="<?php echo $k;?>" <?php if($o==$k) { ?> selected="selected"<?php } ?>><?php echo $v;?></option>
<?php } } ?>
</select>
</div>
...
<tbody>
<?php if($arrLists) { ?> <?php if(is_array($arrLists)) { foreach($arrLists as $k => $v) { ?>
<tr>
<td><?php echo $v['id'];?></td>
<td><?php echo $v['order_username'];?></td>
<td><?php echo $v['price'];?></td>
<td>
<?php echo date('Y-m-d H:i:s',$v['order_time']) ?>
</td>
<td><?php echo $arrStatus[$v['order_status']];?></td>
<td>
<?php if(!in_array($v['order_status'] ,array('complete','close','arbitral'))) { ?>
<a href="index.php?do=gy&id=<?php echo $v['seller_uid'];?>&orderId=<?php echo $v['order_id'];?>" data-title="查看订单">查看订单</a>
<?php } else { ?> -- <?php } ?>
</td>
</tr>
<?php } } ?> <?php } else { ?>
...
<div class="clearfix">
<?php if($strPages) { ?>
<ul class="pagination pagination-sm pull-right"><?php echo $strPages;?>
</ul>
<?php } ?>
</div>
</div>
