...
//$dos = $kekezu->_route;
(! empty ( $do ) && (in_array ( $do, $dos ) || file_exists('control/' . $do . '.php'))) and $do or (! $_GET && ! $_POST and $do = $kekezu->_sys_config ['set_index'] or $do = 'index');
if(stristr($_SERVER["REQUEST_URI"],"index.html")){
	$do=index;
}
if($configs['view_wait'] == 1 && $do != 'login' && $do != 'user'){
	header("Location: /wait.php");
}
if($configs['login_wait'] == 1 && $do == 'login'){
	header("Location: /wait.php");
}
if($configs['user_wait'] == 1 && $do == 'user'){
	header("Location: /wait.php");
}
...
/*if($do == 'index'){
	if($gUid)
		header("location:http://www.iyun720.com/index.php?do=seller&id=$gUid");	
	else
		header("location:http://www.iyun720.com/index.php?do=login");	
}*/
