...
class keke_core_class extends keke_base_class {
	...
	static function favorableRate($uid,$role){
		$uid = intval($uid);
		if(!$uid){
			return 0;
		}
		$info = db_factory::get_one("SELECT buyer_good_num,buyer_total_num,seller_good_num,seller_total_num FROM `".TABLEPRE."witkey_space` WHERE uid = ".intval($uid));
		if($info){
			if($role == '2'){
				$rate = 0;
				if(intval($info['buyer_total_num'])>0){
					$rate = number_format(intval($info['buyer_good_num'])/intval($info['buyer_total_num']),4)*100;
				}
				return $rate;
			}else{
				$rate = 0;
				if(intval($info['seller_good_num'])>0){
					$rate = number_format(intval($info['seller_good_num'])/intval($info['seller_total_num']),4)*100;
				}
				return $rate;
			}
		}
...
class kekezu extends keke_core_class {
	...
	function init_config() {
		...
		define('SMS_ACCOUNT',$config_arr['mobile_username']);
		define('SMS_PASSWD', $config_arr['mobile_password']);
		if($config_arr['open_im'] == '1'){
			define('OPEN_IM',true);
		}else{
			define('OPEN_IM',false);
		}
		$this->_sys_config = $config_arr;
		if(is_array($this->upload_param())){
			define ( 'QN_UPLOAD_OPEN', TRUE );
			define ( 'REVIEW_BACKEND_PATH', '' );
		}else{
			define ( 'QN_UPLOAD_OPEN', FALSE );
			define ( 'REVIEW_BACKEND_PATH', '../' );
		}
		if (( int ) KEKE_DEBUG == 1) {
	...
	function init_user() {
		if ($_SESSION ['uid']) {
		    ...
		    $token = $_COOKIE ['keke_auto_login'];
			$loginInfo = unserialize ( $_COOKIE ['keke_auto_login'] );
			$pwdInfo = explode ( '|', base64_decode ( $loginInfo [2] ) );
			$uInfo = kekezu::get_table_data ( '*', 'witkey_space', " username='$pwdInfo[2]' and token = '$token'", '', '' );
	...
	function upload_param(){
		if($this->_sys_config['upload_type'] == '2'){
			$this->_sys_config['access_key'] 	and $this->_upload_params ['qn_access_key'] = $this->_sys_config['access_key'];
			$this->_sys_config['secret_key'] 	and $this->_upload_params ['qn_secret_key'] = $this->_sys_config['secret_key'];
			$this->_sys_config['qn_bucket'] 	and $this->_upload_params ['qn_bucket'] 	= $this->_sys_config['qn_bucket'];
			$this->_sys_config['qn_domain'] 	and $this->_upload_params ['qn_domain'] 	= $this->_sys_config['qn_domain'];
		}
		if(!$this->_upload_params ['qn_access_key'] || !$this->_upload_params ['qn_secret_key'] || !$this->_upload_params ['qn_bucket'] || !$this->_upload_params ['qn_domain'] ){
			return false;
		}
		return $this->_upload_params;
	}
	...
	function include_qiniu_file(){
		include S_ROOT.'/include/cloudstorage/qiniu/conf.php';
		include S_ROOT.'/include/cloudstorage/qiniu/http.php';
		include S_ROOT.'/include/cloudstorage/qiniu/auth_digest.php';
		include S_ROOT.'/include/cloudstorage/qiniu/fop.php';
		include S_ROOT.'/include/cloudstorage/qiniu/io.php';
		include S_ROOT.'/include/cloudstorage/qiniu/pfop.php';
		include S_ROOT.'/include/cloudstorage/qiniu/resumable_io.php';
		include S_ROOT.'/include/cloudstorage/qiniu/rs_utils.php';
		include S_ROOT.'/include/cloudstorage/qiniu/rs.php';
		include S_ROOT.'/include/cloudstorage/qiniu/rsf.php';
		include S_ROOT.'/include/cloudstorage/qiniu/utils.php';
		include S_ROOT.'/include/cloudstorage/QiniuClass.php';
	}
