...
class keke_register_class {
	...
	function user_register($reg_username, $reg_pwd, $reg_email, $reg_code, $check_code = true, $old_pwd = null,$is_oauth=null) {
		global $kekezu;
		global $_lang;
		$reg_uid = null;
		...
		if ($kekezu->_sys_config ['user_intergration']==2) {
			...
			if($reg_uid<0){
				...
				kekezu::show_msg ( $msg, NULL, 3, NULL, 'warning' );die();
			}
		}
	...
	function check_all($reg_username, $reg_email, $reg_code,$is_oauth=null) {
		global $_lang,$_K;
		$res1 = $this->check_ip ();
		if($_K['do']){
