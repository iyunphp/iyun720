...
abstract class keke_report_class {
	...
	public static function accept_notify($report_info, $user_info, $re_obj) {
		...
		$result = array (
				...
				$_lang ['order_rights_id'] => $report_info ['report_id'],
				$_lang ['order_rights_name'] => $trans_name,
				$_lang ['order_rights_type'] => $re_type,
				$_lang ['order_rights_object'] => $re_obj,
				$_lang ['action'] => $_lang ['attention_follow'] 
		);
		keke_msg_class::notify_user ( $user_info ['uid'], $user_info ['username'], 'transrights_accept', $trans_name . $_lang ['accepted_notice'], $result, 1 );
	}
	public static function process_notify($process_type = 'pass', $report_info, $user_info, $to_userinfo, $process_result, $process_reply = '') {
		global $_lang;
		if ($report_info ['report_type'] != '1') {
			$trans_name = self::get_transrights_name ( $report_info ['report_type'] ); 
			$re_type = self::get_transrights_obj ( $report_info ['obj'] ); 
			if ($process_type == 'pass') { 
				$result = array (
						...
						$_lang ['order_rights_id'] => $report_info ['report_id'],
						$_lang ['order_rights_name'] => $trans_name,
						$_lang ['process_result'] => $process_result 
				);
				$result2 = array (
						'用户名' => $user_info ['username'],
						'网站名称' => $kekezu->_sys_config['website_name'],
				...
				keke_msg_class::notify_user ( $user_info ['uid'], $user_info ['username'], 'transrights_pass', $trans_name . $_lang ['process_notice'], $result, 1 );
				keke_msg_class::notify_user ( $to_userinfo ['uid'], $to_userinfo ['username'], 'reported_pass', $trans_name . $_lang ['process_notice'], $result2, 1 );
			}
			if ($process_type == 'nopass') { 
				if ($process_reply) {
					$process_result .= '　　原因：' . $process_reply;
				}
				$result = array (
						'用户名' => $user_info ['username'],
						'网站名称' => $kekezu->_sys_config['website_name'],
				...
				$result2 = array (
						'用户名' => $to_userinfo ['username'],
						'网站名称' => $kekezu->_sys_config['website_name'],
						$_lang ['order_rights_id'] => $report_info ['report_id'],
						$_lang ['order_rights_name'] => $trans_name,
						$_lang ['process_result'] => $process_result 
				);
				keke_msg_class::notify_user ( $user_info ['uid'], $user_info ['username'], 'transrights_nopass', $trans_name . $_lang ['process_notice'], $result, 1 );
				keke_msg_class::notify_user ( $to_userinfo ['uid'], $to_userinfo ['username'], 'reported_nopass', $trans_name . $_lang ['process_notice'], $result2, 1 );
			}
		}
	}
	public static function process_freeze($report_id, $report_type, $report_satus, $to_uid, $obj, $obj_id, $origin_id, $desc) {
		...
		if ($report_type == '1') {
			...
			if ($report_satus == '1') { 
				...
				if ($res) { 
					$result = array (
							$_lang ['launch_people'] => $_lang ['you'],
							$_lang ['order_rights_object'] => $re_obj,
							$_lang ['order_rights_type'] => $re_type,
							$_lang ['submit_reason'] => $desc 
					);
					$result2 = array (
							'用户名' => $to_user_info ['username'],
							'网站名称' => $kekezu->_sys_config['website_name'],
							$_lang ['order_rights_id'] => $report_id,
							$_lang ['order_rights_name'] => $_lang ['rights'],
							$_lang ['order_rights_type'] => $re_type,
							$_lang ['order_rights_object'] => $re_obj,
							$_lang ['action'] => $_lang ['freeze'] 
					);
					keke_msg_class::notify_user ( $user_info ['uid'], $user_info ['username'], 'transrights_freeze', $re_type . $_lang ['freeze_notice'], $result, 1 );
					keke_msg_class::notify_user ( $to_user_info ['uid'], $to_user_info ['username'], 'transrights_accept', $re_type . $_lang ['freeze_notice'], $result2, 1 );
				}
			}
		}
	}
	...
	public function process_unfreeze($process_type = 'pass', $process_result = null) {
		...
		if ($this->_report_info ['report_status'] == '2') { 
			...
			if ($res) { 
				if ($process_type == 'pass') { 
					$result = array (
							$_lang ['order_rights_id'] => $this->_report_id,
							$_lang ['order_rights_name'] => $_lang ['rights'],
							$_lang ['process_result'] => $process_result 
					);
	...
	public function process_can_init() {
		...
		switch ($this->_report_type) {
			...
			case "2" : 
				if ($this->_obj == 'work') {
					if (! in_array ( $this->_obj_info ['obj_status'], array (
							'0',
							'5',
							'7',
							'8' 
					) ) && $this->_obj_info ['origin_status'] <= 3) {
