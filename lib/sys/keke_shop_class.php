...
class keke_shop_class {
		...
		if ($order_id) {
			if($service_info ['model_id'] == 7){
				...
				if($service_info['model_id'] != '6'&&$step != 'step1'){
				    ...
				    $v_arr = array ($_lang ['user_msg'] => $order_info ['order_username'],  '作品标题' => $order_info ['order_name'], $_lang ['order_link'] => $b_order_link ,'作品模型名称'=>$shopMx);
	...
	//改造 添加参数$key,调取第一张图片，
	static function output_pics($path, $pre = null, $show = false, $key = 'all',$id=0) {		
		if(preg_match('/^http/', $path)){
		    $tmp = explode (',', $path . ',' );
			
			if(intval($id) > 0){
				//调取全景缩略图
				//thumbnail_iyun720_1449912404000_44369769.jpg
				$filename = explode("/",$path);
				$filename = array_slice($filename,-1,1);
				$n = explode('.',$filename[0]);
				$new_filename = 'thumbnail_'.$n[0];
				$type =  array_slice(explode('.',$filename[0]),-1,1);		
				
				$video_type=array('mp4','mov');
				if(in_array($type[0],$video_type)){
					$type[0] = 'jpg';
				}
				$filenames = md5($new_filename).".".$type[0];
				
				if($id == 68){
					
					//print_r($new_filename);
					//echo md5($new_filename);
					//echo md5('thumbnail_'.array_slice($filename),-1,1);
				}
				return "http://view.iyun720.com/pic/".$id."/".$filenames."?r=".date('Ymdh');
			}
			if($key == 1){
				return 	$tmp[0];
