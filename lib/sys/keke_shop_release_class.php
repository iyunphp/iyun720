...
abstract class keke_shop_release_class {
	...
	public function update_service_info($service_id, $obj_name) {
		...
		if ($service_id) {
			...
			if (!empty($release_info ['payitem']) ) {
				...
				kekezu::show_msg ( '该作品的增值服务需要支付！', "index.php?do=pubgoods&id=" . $service_obj->_model_id . "&serviceId=".$service_id."&step=step4&orderId=" . $orderId, 3, null, 'ok' );
			} else {
	...
	public static function updateEditStatusBySid($service_id,$status) {
		return db_factory::execute("UPDATE `".TABLEPRE."witkey_service` SET `edit_status`='{$status}' WHERE (`service_id`='{$service_id}')");
	}
	...
	public static function updateShopStatus($uid,$status){
		db_factory::execute("update ".TABLEPRE."witkey_space set autoshop=1 where uid='{$uid}'");
		return db_factory::execute("UPDATE `".TABLEPRE."witkey_shop` SET `shop_status`='{$status}' WHERE (`uid`='{$uid}')");
	}
