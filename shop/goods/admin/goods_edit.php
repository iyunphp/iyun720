...
if ($op == 'basic') { 
	...
	foreach($service_info['ext_fields'] as $k=>$v){
		if(count(explode('###',$v['f_warning']))>1){
			$service_info['ext_fields'][$k]['f_warning'] = explode('###',$v['f_warning']);
			$service_info['ext_fields'][$k]['f_warning_select']  = 1;
		}else{
			$service_info['ext_fields'][$k] = $v;
		}				
	}
	...
	if($sbt_edit){
		...
		createXML($service_id,$_POST['filepath']);//改造 生成作品xml
