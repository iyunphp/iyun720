...
switch ($ord) {
	...
	case 'default':
	default:
		$wh .= " ORDER BY ( CASE service_status WHEN 1 THEN 0 ELSE 1 END ), ( CASE edit_status WHEN 1 THEN 0 ELSE 1 END ), is_top DESC,on_time desc";
	break;
}
...
if ($ac) {
	...
	switch ($ac) {
		...
		case 'shelves' : 
			...
			createXML($service_id);//改造 生成作品xml
		...
		case 'off_shelf' : 
			if ($is_submit == "1") {
				...
				$objMsgM->setTitle ( "作品下架" );
				$objMsgM->setContent ( kekezu::str_filter ( kekezu::escape ( "您的作品 " . $serviceInfo [title] . " 被管理员下架，下架原因：$reason" ) ) );
				$objMsgM->setOn_time ( time () );
				$objMsgM->setType ( 2 );
				$objMsgM->create_keke_witkey_msg ();
				goods_shop_class::set_service_status ( $service_id, 3 );
				kekezu::admin_show_msg ( $_lang ['operate_notice'], $url_str, 2, '作品下架成功', 'success' );
			} else {
			...
			break;
		case 'recommend' : 
			$sql = sprintf ( "update  %switkey_service set is_top=1 where service_id =%d", TABLEPRE, $service_id );
			db_factory::execute ( $sql );
			kekezu::admin_system_log ( $_lang ['recommend'] . $memberinfo_arr ['username'] );
			kekezu::admin_show_msg ( $_lang ['operate_success'], $url_str, 3, '', 'success' );
			break;
		case 'move_recommend' : 
			$sql = sprintf ( "update  %switkey_service set is_top=0 where service_id =%d", TABLEPRE, $service_id );
			db_factory::execute ( $sql );
