...
switch ($step) {
	case 'step1':
		...
		if (isset($formhash) && kekezu::submitcheck($formhash)) {
			...
			if ($floatPrice < $floatMinCash) {
				$tips['errors']['txt_price'] = '你的作品价格不能少于￥'.$floatMinCash.'元';
				kekezu::show_msg($tips,NULL,NULL,NULL,'error');
			}
			...
			//db_factory::execute("update iyun_witkey_service set areatext = '" . $_POST['area'] . "' where service_id ='" . $intSid . "'");
	...
	case 'step2':
		if (isset($formhash)&&kekezu::submitcheck($formhash)) {
			...
			if($arrPubInfo['area'])
			{
				db_factory::execute("update iyun_witkey_service set areatext = '" . $arrPubInfo['area'] . "' where service_id ='" . $intSid . "'");			
			}
			$objRelease->update_service_info ( $intSid, $stdCacheName ); 	
			createXML($intSid);//改造 生成作品xml
			kekezu::show_msg($tips,$strUrl.'&step=step3&serviceId='.$intSid,NULL,NULL,'ok');

		}else{
			if(!$_SESSION[$stdCacheName])
			{
				kekezu::show_msg($_lang ['friendly_notice'],"index.php?do=pubgoods&id=$id",2,"该作品已提交，不可再返回修改！","warning");
				break;
			}
	...
	case 'step3':
		...
		createXML($serviceId);//改造 生成作品xml
		if(0 === $serviceId){
