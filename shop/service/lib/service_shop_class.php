...
class service_shop_class {
		...
		if ($res) {
			if (is_array ( $service_ids )) {
				...
				$feed_arr = array (
						...
						"event" => array (
								"content" => "$service_info[title]",
								"url" => "index.php?do=goods&id=$service_ids",
								"cash" => "$service_info[price]",
								"model_id" => "$service_info[model_id]" 
						) 
				);
				...
				switch ($action) {
					case "wait" : 
						...
						if ($res) {
							if ($order_info ['obj_type'] == 'gy') {
								...
								$v_arr = array (
										'作品模型名称'=> $arrModel[$service_info['model_id']],
										'作品标题'=>$service_info['title'] ,
										$_lang ['user_msg'] => $order_info ['seller_username'],
										$_lang ['action'] => $_lang ['recept_your_order'],
										$_lang ['order_id'] => $order_id,
										$_lang ['order_link'] => $b_order_link 
								);
								keke_shop_class::notify_user ( $order_info ['order_uid'], $order_info ['order_username'], "order_create", $_lang ['goods_order_recept'], $v_arr, 1 );
							}
							if ($isApp) {
					...
					case "ok" : 
						if ($order_info ['obj_type'] == 'gy') {
							...
							keke_shop_class::notify_user ( $order_info ['order_uid'], $order_info ['order_username'], "order_change", $_lang ['goods_order_recept'], $v_arr, 1 );
						}
						...
						if ($suc) {
							...
							if ($order_info ['obj_type'] == 'gy') {
								...
								keke_shop_class::notify_user ( $order_info ['seller_uid'], $order_info ['seller_username'], "order_change", $_lang ['goods_order_confirm_pay'], $v_arr, 1 );
							}
							if ($isApp) {
								app_class::response ( array (
										'ret' => 1010,
					...
					case "close" : 
						...
						if ($res) {
							...
							if ($order_info ['obj_type'] == 'gy') {
								...
								keke_shop_class::notify_user ( $order_info ['order_uid'], $order_info ['order_username'], "order_change", $_lang ['goods_order_close'], $v_arr, 1 );
							}
							if ($isApp) {
								app_class::response ( array (
										'ret' => 1012,
					...
					case 'over_time_close' :
						...
						if ($res) {
							...
							if ($order_info ['obj_type'] == 'gy') {
								...
								keke_shop_class::notify_user ( $order_info ['seller_uid'], $order_info ['seller_username'], "order_change", '超时未完成服务', $v_arr, 1 );
							}
						}
						break;
					case "confirm_complete" : 
						...
						if ($res) {
							...
							if ($order_info ['obj_type'] == 'gy') {
								...
								keke_shop_class::notify_user ( $order_info ['order_uid'], $order_info ['order_username'], "order_change", '服务完成', $v_arr, 1 );
							}
							if ($order_info ['obj_type'] == 'gy') {
					...
					case "send" : 
						...
						if ($res) {
							if ($order_info ['obj_type'] == 'gy') {
								...
								$v_arr = array (
										$_lang ['user_msg'] => $order_info ['seller_username'],
										$_lang ['action'] => $_lang ['confirm_service_complete'],
										$_lang ['order_id'] => $order_id,
										$_lang ['order_link'] => $b_order_link 
								);
								keke_shop_class::notify_user ( $order_info ['order_uid'], $order_info ['order_username'], "order_change", $_lang ['service_order_confirm_complete'], $v_arr, 1 );
							}
							if ($isApp) {
								app_class::response ( array (
										'ret' => 1018,
					...
					case "complete" : 
						...
						if ($res) {
							...
							if ($order_info ['obj_type'] == 'gy') {
								...
								keke_shop_class::notify_user ( $order_info ['seller_uid'], $order_info ['seller_username'], "order_change", $_lang ['service_order_confirm_complete'], $v_arr, 1 );
							}
							if ($isApp) {
								app_class::response ( array (
										'ret' => 1020,
						...
						if ($res) {
							if ($order_info ['obj_type'] == 'gy') {
								...
								if ($uid == $order_info ['order_uid']) {
									...
									keke_shop_class::notify_user ( $order_info ['seller_uid'], $order_info ['seller_username'], "order_change", $_lang ['sevice_order_arbitrate_submit'], $v_arr, 1 );
								} else {
									$v_arr = array (
											$_lang ['user_msg'] => $order_info ['seller_username'],
									...
									keke_shop_class::notify_user ( $order_info ['order_uid'], $order_info ['order_username'], "order_change", $_lang ['sevice_order_arbitrate_submit'], $v_arr, 1 );
								}
							}
							if ($isApp) {
								app_class::response ( array (
					...
					case "delete" : 
						...
						if ($isApp) {
							$res and app_class::response ( array (
									'ret' => 1006 
							) ) or app_class::response ( array (
									'ret' => 1007 
							) );
						} else {
	...
	public static function sendNoticeToBuyer($order_info, $statusText) {
		...
		$msg_obj->send_message ( $order_info ['order_uid'], $order_info ['order_username'], 'gy_notice_to_buyer', '雇佣状态消息', $v_arr, null, null, 1 );
	}
	public static function sendNoticeToSeller($order_info, $statusText) {
		...
		$msg_obj = new keke_msg_class (); 
		$msg_obj->send_message ( $order_info ['seller_uid'], $order_info ['seller_username'], 'gy_notice_to_seller', '雇佣状态消息', $v_arr, null, null, 1 );
	}
}
?>