/**
* 以百度上传组件为基础二次封装 插件
* @file  kppw.webuploader.js
* @author liwei <phper_lee@foxmail.com>
* @time  2015-9-6 下午2:26:11
			...
			var self = this;
				...
				if(!self.options.uploadEvents.uploadProgress){
					self.BDUploader.on('uploadProgress',function(file, percentage){																			
						var $li = $( '#'+file.id ),
							...
							if(ext_arr.indexOf(file.ext) == -1 && self.options['hiddenName'] == 'file_path_2' && file['_info'].width/file['_info'].height != 2){
								self.BDUploader.removeFile( file );								
								swal('未知类型上传错误','您上传的非(2:1)全景图片，请重新上传！','error');
				...
				if(!self.options.uploadEvents.uploadError){
					self.BDUploader.on('uploadError',function(file ,reason){
						self.dump(file);
						self.dump(reason);
						swal('图片解析失败！','uploadError uploadError: reason is undefined!','error');
	...
	$.fn[pluginName].defaults = {
		...
		/** 
		 ...
		 thumb				:{},//配置生成缩略图的选项。
			 ...
			 */
		 compress 			:{width: 0,
				    height: 0},// {Object} [可选]		 配置压缩的图片的选项。如果此选项为false, 则图片在上传前不进行压缩。
			/**
				 * {
				    width: 1600,
				    height: 1600,
	     ...
	     server			: 'index.php?do=ajax&view=upload&flash=1',// 文件接收服务端。
	     /**
		 ...
		 delUrl			: 'index.php?do=ajax&view=upload&action=delete',
		 separator		: ',', //默认逗号
	}
