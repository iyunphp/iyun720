...
class sreward_task_class extends keke_task_class {
		...
		if ($this->_task_status == 3 && $this->_task_info ['end_time'] < time ()) {
			if ($this->_task_info ['work_num'] > 0) {
				...
				if ($rw_count == '1') {
					...
					keke_msg_class::notify_user ( $this->_guid, $this->_gusername, 'timeout', $_lang ['timeout_sys_default_in_and_bid'], $v_arr,1 );
				} elseif ($rw_count > 1) {
					...
					keke_msg_class::notify_user ( $this->_guid, $this->_gusername, 'timeout', $_lang ['timeout_sys_default_vote_status'], $v_arr,1 );
				} else {
					$this->auto_choose ();
				}
			} else {
	...
	public static function master_opera($m_id, $t_id, $url, $t_cash) {
		...
		$arrTaskInfo = db_factory::get_one ( sprintf ( ' select * from %switkey_task where task_id=%d and uid=%d', TABLEPRE, $t_id, $uid ) );
	...
	public static function getUsername($uid){
		$strSql="select * from ".TABLEPRE."witkey_space where uid=".intval($uid);
		$arrResult=db_factory::get_one($strSql);
		return $arrResult['username'];
	}
	public static function getTitle($task_id){
		$strSql="select * from ".TABLEPRE."witkey_task where task_id=".intval($task_id);
		$arrResult=db_factory::get_one($strSql);
		return $arrResult['task_title'];
	}
